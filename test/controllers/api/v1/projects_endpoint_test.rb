...
class Api::V1::ProjectsEndpointTest < Api::Test
  include Devise::Test::IntegrationHelpers
    ...
    def assert_proper_object_serialization(project_data)
      ...
      assert_equal project_data['name'], project.name
      assert_equal project_data['tag_ids'], project.tag_ids
    ...
    test "index" do
      ...
      project_ids_returned = response.parsed_body.dig("data").map { |project| project.dig("attributes", "id") }
    ...
    test "create" do
      ...
      project_data = Api::V1::ProjectSerializer.new(build(:project, team: nil)).serializable_hash.dig(:data, :attributes)
      project_data.except!(:id, :team_id, :created_at, :updated_at)

      post "/api/v1/teams/#{@team.id}/projects",
        params: project_data.merge({access_token: access_token})
      ...
      # Also ensure we can't do that same action as another user.
      post "/api/v1/teams/#{@team.id}/projects",
        params: project_data.merge({access_token: another_access_token})
    ...
    test "destroy" do
      ...
      # Also ensure we can't do that same action as another user.
      delete "/api/v1/projects/#{@project.id}", params: {access_token: another_access_token}
      assert_response_specific_not_found
    end
end
